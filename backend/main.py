...
async def process_sheet(file: UploadFile = File(...), suc: str = "", examID: str = "") -> JSONResponse:
    ...
    input_path = os.path.join(UPLOAD_DIR, f"{temp_name}{file_ext}")
    
    with open(input_path, "wb") as f:
        f.write(await file.read())
    
    if file_ext == ".pdf":
        images = convert_from_path(input_path, dpi=300)  # list of PIL images
        # take first page only (or loop if multiple pages needed)
        pdf_img_path = os.path.join(UPLOAD_DIR, f"{temp_name}_page1.jpg")
        images[0].save(pdf_img_path, "JPEG")
        convert_path = convert_mobile_to_pc(pdf_img_path, pc_out=pdf_img_path)
    else:
...
@app.post("/uploadfile")
async def uploadfile(file:UploadFile = File(...),suc:str = None, examid:str = None):
    # store the uploadfiles
    exam_dir = os.path.join(LOAD_DIR,examid)
    os.makedirs(exam_dir,exist_ok=True)
    _,file_ext = os.path.splitext(file.filename)
    new_filename = f"{suc}{file_ext}" if suc else file.filename
    file_path = os.path.join(exam_dir,new_filename)
...
@app.post("/path_process")
async def path_process(filepath: str):
    # Extract filename and extension
    examid = os.path.basename(os.path.dirname(filepath))

    # Extract filename and extension
    filename_with_ext = os.path.basename(filepath)
    filename, ext = os.path.splitext(filename_with_ext)

    suc = filename  # since filename itself is suc

    # If PDF, convert first page to image
    if ext.lower() == ".pdf":
        images = convert_from_path(filepath, dpi=300)
        pdf_img_path = os.path.join(UPLOAD_DIR, f'{filename}.jpg')
        images[0].save(pdf_img_path, "JPEG")
        convert_path = convert_mobile_to_pc(pdf_img_path, pc_out=pdf_img_path)
    else:
        # Use the file directly
        convert_path = convert_mobile_to_pc(filepath, pc_out=filepath)
    ...
    # Crop and analyze
    cropped = crop_sheet(image)
    result, labeled_path = analyze_sheet(cropped, UPLOAD_DIR, filename)
    ...
    # logger.info(f"Processed: {filename}, JSON saved to {json_path}")
    logger.info(f"image_url: /uploads/{os.path.basename(labeled_path)}")
